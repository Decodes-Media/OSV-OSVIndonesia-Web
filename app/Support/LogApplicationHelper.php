...
class LogApplicationHelper
{
    ...
    public static function getAllForFile(string $file): array
    {
        ...
        if (File::size($file) > self::MAX_FILE_SIZE) {
            throw new Exception(__('File size is to large'));
        }
        ...
        foreach ($headings as $h) {
            for ($i = 0, $j = count($h); $i < $j; $i++) {
                foreach (self::LOG_LEVELS as $level) {
                    $lower = strtolower($h[$i]);
                    if (strpos($lower, '.'.$level) || strpos($lower, $level.':')) {
                        $pattern = '/^\[(?P<date>(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}))\]'
                                   .'(?:.*?(?P<context>(\w+))\.|.*?)'.$level
                                   .': (?P<text>.*?)(?P<in_file> in .*?:[0-9]+)?$/i';
    ...
    public static function pathToLogFile(string $file): string
    {
        ...
        if (dirname($file) !== $logsPath) {
            throw new Exception(__('No such file or directory'));
        }
